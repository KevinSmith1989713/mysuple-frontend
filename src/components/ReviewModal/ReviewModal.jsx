...
const ReviewModal = ({
	...
	useEffect(() => {
		setScore({
			fun_score: !!info.review
				? info.review.fun_score
				: !!cardInfo && cardInfo.fun_score,
			complete_score: !!info.review
				? info.review.complete_score
				: !!cardInfo && cardInfo.complete_score,
			difficulty_score: !!info.review
				? info.review.difficulty_score
				: !!cardInfo && cardInfo.difficulty_score,
			operate_score: !!info.review
				? info.review.operation_score
				: !!cardInfo && cardInfo.operation_score,
		});
		// setTotal(!!info.review ? info.review.total_score : 0);
		// setEveluate(!!info.review ? info.review.evaluation_content : '');
		return () => {};
	}, [isOpen]);

	const [scores, setScore] = useState({
		fun_score: !!info.review ? info.review.fun_score : '',
		complete_score: !!info.review ? info.review.complete_score : '',
		difficulty_score: !!info.review ? info.review.difficulty_score : '',
		operate_score: !!info.review ? info.review.operation_score : '',
	});
	// const [total_score, setTotal] = useState(
	// 	!!info.review ? info.review.total_score : 0,
	// );
	const [total_score, setTotal] = useState(
		0 || (!!cardInfo && cardInfo.total_score),
	);
	...
	const onClickSubmit = () => {
		if (profileSubMenu === 'myGame') {
			const review_id = !!cardInfo && cardInfo.review_id;
			insertReviewUpdate(
