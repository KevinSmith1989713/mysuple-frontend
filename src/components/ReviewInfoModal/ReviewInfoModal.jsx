...
const ReviewInfoModal = ({
	...
	useEffect(() => {
		setScore({
			fun_score: info.my_review && info.my_review[0].fun_score,
			complete_score: info.my_review && info.my_review[0].complete_score,
			difficulty_score: info.my_review && info.my_review[0].difficulty_score,
			operate_score: info.my_review && info.my_review[0].operation_score,
		});
		setEvaluation_content_myGame(
			info.my_review && info.my_review[0].evaluation_content,
		);
		setTotal(info.my_review && info.my_review[0].total_score);
	}, [isOpen]);

	// console.log(info.my_review && info.my_review[0]);
	...
	const [total_score, setTotal] = useState(
		info.my_review && info.my_review[0].total_score,
	);
	...
	// console.log('찐찐', evaluation_content_myGame);
	...
	const onClickSubmit = () => {
		if (info.my_review !== null) {
			const review_id = info.my_review && info.my_review[0].review_id;
			insertReviewUpdate(
	...
	// console.log(info.my_review && info.my_review[0].evaluation_content);
	...
	return (
		<Modal className="ReviewModal" close={close} isOpen={isOpen}>
			...
			<div className="ReviewModal--GameInfo">
				<img className="poster" src={!!info && info.img_src} />
				<div className="info--section">
					...
					<div className="tag">
						<TextWithIcon icon={tag}>
							{!!info.game_tag_kr &&
								info.game_tag_kr.split(',').map((tag, index) => {
									if (index < 9)
										return (
				...
				<div className="ReviewModal--StarRating">
					...
					<div className="Star--rating">
						<StarRating
							...
							totalScore={
								total_score || (info.my_review && info.my_review[0].total_score)
							}
						/>
...
export default connect(mapStateToProps, mapDispatchToProps)(ReviewInfoModal);
